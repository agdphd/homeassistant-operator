...
| metricsNetworkPolicy.enabled | bool | `false` | Restrict ingress to the operator pod: its `/metrics` endpoint to namespaces labeled `metrics: enabled`, and — when `webhook.enabled` is also true — its admission webhook to namespaces labeled `webhook: enabled`. Egress is deliberately not restricted, because there is no portable way to address the Kubernetes API server across cluster types and CNIs from a generic chart. Unrelated to `spec.alpha.networkPolicy.enabled` on a HomeAssistant resource, which protects the Home Assistant pod instead. |
| nameOverride | string | `""` | Override the chart name used in resource names. |
| namespace.create | bool | `false` | Render the release Namespace and label it to enforce the "restricted" Pod Security Standard (version latest). Install WITHOUT `--create-namespace` so the chart owns the namespace. When false (the default), you manage the namespace and no Pod Security Admission labels are added — the operator pod is restricted-compliant either way, see `securityContext`. |
