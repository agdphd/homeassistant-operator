...
// +kubebuilder:rbac:groups=ha.homeassistant.io,resources=homeassistantsecrets,verbs=get;list;watch;create;update;patch;delete
...
func (r *HomeAssistantSecretsReconciler) collectSecrets(ctx context.Context, haSecrets *hav1alpha1.HomeAssistantSecrets) (map[string]string, error) {
	log := logf.FromContext(ctx)
...
func (r *HomeAssistantSecretsReconciler) generateSecretsYaml(secretsData map[string]string) string {
	...
	sb.WriteString("# Auto-generated by HomeAssistantSecrets controller\n")
...
// reconcileGeneratedSecret creates or updates the generated Secret containing secrets.yaml
func (r *HomeAssistantSecretsReconciler) reconcileGeneratedSecret(ctx context.Context, haSecrets *hav1alpha1.HomeAssistantSecrets, secretsYaml, hash string) error {
	log := logf.FromContext(ctx)
	...
	err := r.Get(ctx, types.NamespacedName{Name: secretName, Namespace: haSecrets.Namespace}, existingSecret)
	if err != nil {
		if errors.IsNotFound(err) {
			// Create new secret
	...
	if existingSecret.Annotations[secretsHashAnnotationKey] != hash {
		log.Info("Updating generated secret (hash changed)", "name", secretName, "oldHash", existingSecret.Annotations[secretsHashAnnotationKey], "newHash", hash)
...
func (r *HomeAssistantSecretsReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(
			&corev1.Secret{},
			handler.EnqueueRequestsFromMapFunc(r.findHomeAssistantSecretsForSecret),
		).
		Named("homeassistantsecrets").
		Complete(r)
}

// findHomeAssistantSecretsForSecret finds all HomeAssistantSecrets that reference a given Secret
func (r *HomeAssistantSecretsReconciler) findHomeAssistantSecretsForSecret(ctx context.Context, obj client.Object) []reconcile.Request {
	secret := obj.(*corev1.Secret)

	// List all HomeAssistantSecrets in the same namespace
	haSecretsList := &hav1alpha1.HomeAssistantSecretsList{}
	if err := r.List(ctx, haSecretsList, client.InNamespace(secret.Namespace)); err != nil {
		return []reconcile.Request{}
	}

	var requests []reconcile.Request
	for _, haSecrets := range haSecretsList.Items {
		// Check if this HomeAssistantSecrets references the changed Secret
		for _, secretRef := range haSecrets.Spec.SecretRefs {
...
// updateStatefulSetAnnotation updates the secrets hash annotation on the StatefulSet
// to trigger a rolling restart when secrets change
func (r *HomeAssistantSecretsReconciler) updateStatefulSetAnnotation(ctx context.Context, haSecrets *hav1alpha1.HomeAssistantSecrets, hash string) error {
	log := logf.FromContext(ctx)
