...
func newScriptValidator(t *testing.T, objs ...client.Object) *HomeAssistantScriptCustomValidator {
	t.Helper()
	...
	return &HomeAssistantScriptCustomValidator{Client: cl}
}

func script(name, id, haRef string) *hav1.HomeAssistantScript {
...
func TestHomeAssistantScriptCustomValidator_Validate(t *testing.T) {
	t.Run("no siblings admits", func(t *testing.T) {
		v := newScriptValidator(t)
		if _, err := v.ValidateCreate(context.Background(), script("first", "foo", "home")); err != nil {
			t.Errorf("unexpected rejection: %v", err)
		}
	})

	t.Run("explicit id collision with existing sibling is rejected and names it", func(t *testing.T) {
		existing := script("first-script", "backup_now", "home")
		v := newScriptValidator(t, existing)
		incoming := script("second-script", "backup_now", "home")
		_, err := v.ValidateCreate(context.Background(), incoming)
		if err == nil {
			t.Fatal("expected rejection, got nil error")
		}
		if !strings.Contains(err.Error(), "first-script") {
			t.Errorf("error %q does not name the conflicting sibling %q", err.Error(), "first-script")
		}
	})
	...
	t.Run("name-fallback id collision is rejected", func(t *testing.T) {
		...
		existing := script("sharedname", "", "home")
		v := newScriptValidator(t, existing)
		incoming := script("second-script", "sharedname", "home")
		_, err := v.ValidateCreate(context.Background(), incoming)
		if err == nil {
			t.Fatal("expected rejection, got nil error")
		}
	...
	t.Run("same id, different HomeAssistant instance admits", func(t *testing.T) {
		existing := script("first", "foo", "home")
		v := newScriptValidator(t, existing)
		incoming := script("second", "foo", "away")
	...
	t.Run("sibling marked for deletion is not a conflict", func(t *testing.T) {
		...
		existing.Finalizers = []string{"ha.homeassistant.io/script"}
		v := newScriptValidator(t, existing)
		incoming := script("second", "foo", "home")
		if _, err := v.ValidateCreate(context.Background(), incoming); err != nil {
	...
	t.Run("update does not conflict with its own prior state", func(t *testing.T) {
		self := script("first", "foo", "home")
		v := newScriptValidator(t, self)
